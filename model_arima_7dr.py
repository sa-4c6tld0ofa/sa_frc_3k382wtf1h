...
def get_model_price_arima_7dr(uid,date_str):
    ...
    try:
        ...
        sql = "SELECT price_close FROM price_instruments_data JOIN symbol_list ON symbol_list.symbol = price_instruments_data.symbol WHERE symbol_list.uid = " + str(uid) + " ORDER BY date DESC LIMIT 1"
        cr.execute(sql)
        rs = cr.fetchall()
        last_price = 0
