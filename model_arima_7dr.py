...
def get_model_price_arima_7dr(uid, date_str, connection):
    ...
    " AND price_instruments_data.date = " + str(date_str)
    cursor.execute(sql)
    res = cursor.fetchall()
    last_price = 0
    for row in res:
    ...
    if float(last_price) <= float(point_forecast):
        ret = float(last_price) -  (float(point_forecast) -  float(last_price))
    if float(last_price) > float(point_forecast):
