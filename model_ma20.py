...
def get_model_price_ma20(uid, date_str, connection):
    """
    ...
    sql = "SELECT price_close, "+ ma_column_name +\
...
########################################################################
def set_model_ma20(uid, force_full_update, connection):
    """ xxx """
    ret = 0
    ########################################################################
    # (2.1) Define names of column in use by the model
    ########################################################################
    model_tp_column = 'price_instruments_data.ma20_tp'
    ...
    for row in res:
        ...
        if model_tp == 0:
            ...
            ########################################################################
            last_model_tp = get_model_price_ma20(uid, last_date, connection)
