...
def get_model_price_action_10d(uid, date_str, connection):
    """
    ...
    ################################################
    ret = 0
    column_data_name = 'price_action_10d'
    stdev_st = 0
    symbol = ''
    price_close = 0
    model_data = 0
    ...
    debug(str(symbol) + ' ::: '+ str(ret) +' = ' +\
          str(date_str) + ' ::: ' + str(price_close) +\
          ' ::: stdev=' + str(stdev_st))
    return ret
...
########################################################################
def set_model_price_action_10d(uid, force_full_update, connection):
    """ xxx """
    #-------------------------------------------------------------------
    ret = 0
    ########################################################################
    ...
    ########################################################################
    model_column = 'price_instruments_data.price_action_10d'
    model_tp_column = 'price_instruments_data.price_action_10d_tp'
    ...
    for row in res:
        ...
        if model_tp == 0:
            ...
            ########################################################################
            model_tp = get_model_price_action_10d(uid, last_date, connection)
            debug(str(model_tp) + ' ::: ' + str(last_date) +\
                  ' ::: ' + str(selected_date))
            #-----------------------------------------------------------------------
            cr_v = connection.cursor(pymysql.cursors.SSCursor)
            sql_v = "UPDATE price_instruments_data SET " + str(model_tp_column) +\
            " = " + str(model_tp) + " WHERE symbol = '"+\
