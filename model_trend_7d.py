...
def get_model_7d_trend(uid,date_str):
    ################################################
    ...
    try:
        ...
        sql = "SELECT price_close, 7dtrend FROM price_instruments_data WHERE symbol = '"+ str(symbol) +"' AND date = " + str(date_str)
        cr.execute(sql)
        rs = cr.fetchall()
        for row in rs:
            price_close = row[0]
    ...
    except Exception as e: debug("get_model_price_7d_trend() " + str(e) )
    return r

########################################################################
# (2) Set the name of the model function
########################################################################
def set_model_7d_trend(uid,force_full_update):
    #-------------------------------------------------------------------
    r = 0
    try:
        ########################################################################
        ...
        ########################################################################
        model_tp_column = 'price_instruments_data.7dtrend_tp'
        model_score_column = 'instruments.score_7dtrend'
        #-----------------------------------------------------------------------
        ...
        for row in rs:
            ...
            if model_tp == 0:
                ...
                ########################################################################
                last_model_tp = get_model_7d_trend(uid,last_date)
                #-----------------------------------------------------------------------
        ...
        connection.close()
        gc.collect()
    except Exception as e: debug("set_model_7d_trend() " + str(e) )
    return r
